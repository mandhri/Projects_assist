...
```{r fig.width=15, fig.height=12, message=FALSE, warning=FALSE, echo=TRUE}
...
ages <- intersect(names(pies_shr), names(pies_raw))

# Interleave shrunken/raw per age, then wrap into a 2-col panel
plots <- unlist(lapply(ages, function(a) {
  list(
    pies_shr[[a]] + ggtitle(paste(a, "• Shrunken LFC")),
    pies_raw[[a]] + ggtitle(paste(a, "• Unshrunken LFC"))
  )
}), recursive = FALSE)
