...
# reference bin = main Sex effect
res_main_raw <- results(dds, name = "Sex_Male_vs_Female")
res_shr[[ref_age]] <- lfcShrink(dds, coef = "Sex_Male_vs_Female", type = "ashr")
res_shr[[ref_age]]$pvalue <- res_main_raw$pvalue; res_shr[[ref_age]]$padj <- res_main_raw$padj


# filter AFTER shrinkage
lfc_cut <- 1
res_sig <- lapply(res_shr, \(x) subset(as.data.frame(x), !is.na(padj) & padj < 0.05 & abs(log2FoldChange) > lfc_cut))
