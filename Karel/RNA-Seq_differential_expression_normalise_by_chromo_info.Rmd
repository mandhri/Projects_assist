...
# Effect direction (Higher in males/ females)
...
## A) Build TWO DEG sets per bin under the same padj rule
padj_thr <- 0.05
lfc_thr  <- 1  # match Karel's magnitude cut

# 1) Karel-style: threshold on RAW LFC
deg_karel <- lapply(results_list, function(df) {
  keep <- !is.na(df$padj) & df$padj < padj_thr & abs(df$log2FoldChange) > lfc_thr
  tibble(gene = rownames(df)[keep])
})

# 2) Your style: threshold on SHRUNKEN LFC
deg_shrunk <- lapply(results_list, function(df) {
  keep <- !is.na(df$padj) & df$padj < padj_thr & abs(df$lfc_shr) > lfc_thr
  tibble(gene = rownames(df)[keep])
})

## B) Use ONE mapping method consistently (pick biomaRt here)
library(biomaRt)
ens <- useEnsembl("genes", dataset = "hsapiens_gene_ensembl")
all_ids <- unique(sub("\\..*$","", unlist(lapply(results_list, rownames))))
map_df <- getBM(
  attributes = c("ensembl_gene_id","chromosome_name"),
  filters = "ensembl_gene_id",
  values = all_ids, mart = ens
) %>%
  dplyr::mutate(chromosome_name = dplyr::case_when(
    chromosome_name %in% as.character(1:22) ~ chromosome_name,
    chromosome_name %in% c("X","Y","MT","M") ~ ifelse(chromosome_name=="M","MT",chromosome_name),
    TRUE ~ NA_character_
  )) %>% dplyr::distinct()

map_vec <- setNames(map_df$chromosome_name, map_df$ensembl_gene_id)

## C) Compare pies per bin under the two LFC rules (same mapping)
cmp_one_bin <- function(bin) {
  g1 <- sub("\\..*$","", deg_karel[[bin]]$gene)
  g2 <- sub("\\..*$","", deg_shrunk[[bin]]$gene)

  chr_k <- map_vec[g1]
  chr_s <- map_vec[g2]

  f <- function(chr) {
    grp <- ifelse(chr %in% as.character(1:22), "Autosome",
           ifelse(chr %in% c("X","Y","MT"), chr, "Other"))
    prop.table(table(factor(grp, levels = c("Autosome","X","Y","MT","Other"))))
  }

  data.frame(
    chrom_group = c("Autosome","X","Y","MT","Other"),
    prop_rawlfc = as.numeric(f(chr_k)),
    prop_shrlfc = as.numeric(f(chr_s)),
    bin = bin
  )
}

bins <- names(results_list)
cmp_tbl <- dplyr::bind_rows(lapply(bins, cmp_one_bin))
print(cmp_tbl)
