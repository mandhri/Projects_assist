...
For each bin, the code records the number tested, total significant genes, and directionality (higher in Male vs Female based on shrunken LFC for the shrunken set).
...
After shrinkage (ashr), autosomal borderline hits decrease, so the relative share of X/Y increases among significant genes. Percentages are compositional (depend on how many genes pass filters per bin) and are not length- or background-normalised.
...
For each age bin, tested genes are mapped to {Autosome, X, Y, MT} and tallied per chromosome to form a background. Significant genes (unshrunken and shrunken sets) are then joined to this background to compute chromosome-specific hit rates (sig/tested) and within-bin percentages.
...
# Function for creating panel of normalised pies for one df (raw / shrunken)
...
p_norm_raw <- plot_pies(
  df_rates   = norm_raw,
  df_totals  = tot_raw,
  title_text = "Chromosome distribution by age bin (normalised): Unshrunken",
  classes    = classes,
  pal        = pal,
  ncol_facets = 3
)
...
p_norm_shr <- plot_pies(
  df_rates   = norm_shr,
  df_totals  = tot_shr,
  title_text = "Chromosome distribution by age bin (normalised): Shrunken",
  classes    = classes,
  pal        = pal,
  ncol_facets = 3
)
...
############### Chromosome enrichment (normalised by arm length)############

All tested Ensembl IDs across bins are retrieved and mapped to GRCh38 positions via Ensembl. Midpoints are computed per gene, and UCSC cytoBand (hg38) is used to define centromere spans and obtain p/q arm lengths for chromosomes 1–22, X and Y. Each gene is then assigned to an arm (p or q) by comparing its midpoint to the centromere. Significant genes are aggregated per age bin, chromosome arm and direction (higher in Male/Female), using the appropriate LFC source (raw for the unshrunken set, shrunken for the shrunken set). Counts are normalised by arm length to DEGs per 10 Mb. 
...
# UCSC cytoBand (hg38) to centromeres to arm lengths
...
# Map each gene to its chromo arm
gene_arm <- gene_pos %>%
...
arm_rates_from_list <- function(sig_list, type_label = c("Unshrunken","Shrunken")[1]) {
  ...
  # DEG table with direction per contrast
