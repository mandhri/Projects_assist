...
# Guardrails
stopifnot(sex_main %in% coeffs)
need <- setNames(paste0("age_bin", bins[-1], ".SexMale"), bins[-1])
miss <- setdiff(unname(need), coeffs)
if (length(miss)) stop("Missing interaction terms: ", paste(miss, collapse = ", "))


# build shrunken effects for Sex within each age bin
res_shr <- setNames(vector("list", length(bins)), bins)

# Reference age: main Sex effect
res_raw_ref <- results(dds, name = sex_main)
res_shr[[ref_age]] <- lfcShrink(dds, coef = sex_main, type = "ashr")
res_shr[[ref_age]]$pvalue <- res_raw_ref$pvalue
res_shr[[ref_age]]$padj   <- res_raw_ref$padj

# Other ages: main + interaction contrast
for (age in bins[-1]) {
  itx <- need[[age]]
  r_raw <- results(dds, contrast = list(c(sex_main, itx)))
  r_shr <- lfcShrink(dds, contrast = list(c(sex_main, itx)), type = "ashr")
  r_shr$pvalue <- r_raw$pvalue
  r_shr$padj   <- r_raw$padj
  res_shr[[age]] <- r_shr
}

# Filter after shrinkage
lfc_cut <- 1
res_sig <- lapply(names(res_shr), function(nm) {
  df <- as.data.frame(res_shr[[nm]])
  df$contrast <- nm
  subset(df, !is.na(padj) & padj < 0.05 & abs(log2FoldChange) > lfc_cut)
})
names(res_sig) <- names(res_shr)
