...
res_all2 <- res_all %>%
  mutate(
    ...
    big  = abs(log2FoldChange) >= 0.5,
    hit  = sig & big
  )

ggplot(res_all2, aes(log2FoldChange, -log10(FDR), colour = hit)) +
  ...
  labs(x="Shrunken log2 fold change", y="-log10(FDR)",
       title="Male vs Female within each age bin") +
...
for (age in bins) {
  ...
  df <- data.frame(
    ...
    FDR_raw         = raw_res$padj,             # keep inference from unshrunken fit
    contrast        = age,
    stringsAsFactors = FALSE
  )
