...
dds_org <- DESeqDataSetFromMatrix(countData = samples, colData =pheno , design = ~ Seq_Batch + age_bin + Sex + age_bin:Sex)
...
#Seeing normalisationFactors(dds) return NULL simply means you have raw counts, you either need to call estimateSizeFactors(dds) (or DESeq(dds))
...
#vsd_dds1 %>%         assay() %>%        cor() %>%        pheatmap(annotation = dplyr::select(metadata2, "condition"))
...
dds_2<- DESeq(dds_1)
plotDispEsts(dds_2)
DESeq2::resultsNames(dds_2)
design(dds_2)
lapply(as.data.frame(colData(dds_2)[, c("Seq_Batch","age_bin","Sex")]), \(x) if(is.factor(x)) levels(x) else class(x))
colnames(attr(dds_2, "modelMatrix"))
mm <- model.matrix(design(dds_2), as.data.frame(colData(dds_2)))
colnames(mm)

###############

pheno$Seq_Batch <- factor(pheno$Seq_Batch)
pheno$age_bin   <- factor(pheno$age_bin)
...
dds_2 <- DESeq(DESeqDataSetFromMatrix(samples[rownames(dds_2), ], pheno[colnames(dds_2), ], 
                                      design = ~ Seq_Batch + age_bin + Sex + age_bin:Sex))
#Extract DEGs per age bin


age_bins <- c("a30_39","a40_49","a50_59","a60_69","a70_79")
results_list <- list()
results_list[["a18_29"]] <- results(dds_2, name = "Sex_Male_vs_Female")

for (age in age_bins) {
  int_term <- paste0("age_bin", age, ".SexMale")
  results_list[[age]] <- results(dds_2, contrast = list(c("Sex_Male_vs_Female", int_term)))
}

# Filter for significant DEGs |log2FC| > 1, padj < 0.05
...
#Visualising results - Expression plot

## volcano plots to show the fold changes relative to the adjusted p-values for all genes.
