...
pheno <- tibble::rownames_to_column(pheno, var = "ex_code")
...
dds <- DESeqDataSetFromMatrix(countData = samples, colData =pheno , design = ~ Seq_Batch + age_bin + Sex + age_bin:Sex)
...
vsd_dds1 %>% 
        assay() %>%
        cor() %>%
        pheatmap(annotation = dplyr::select(metadata2, "condition"))
